...
| Memory layer | `agent_server/memory.py` | Per-user async CRUD operations against Lakebase with `UserCredentials` isolation |
...
- Access to a Databricks workspace with:
  ...
  - A Lakebase instance (for memory and chat persistence)
...
│   ├── memory.py             # Per-user Lakebase memory with UserCredentials
...
### Agent Memory (`agent_memory` schema)

| Column | Type | Description |
|--------|------|-------------|
| `id` | SERIAL | Primary key |
| `user_id` | TEXT | User email (e.g., `karthik@databricks.com`) |
